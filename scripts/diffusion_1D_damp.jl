...
@views function diffusion_1D()
    ...
    damp  = 0.14       # damping (this is a tuning parameter, dependent on e.g. grid resolution)
    # Derived numerics
    dx    = lx/nx      # grid size
    ...
    while t<ttot
        ...
        while err>tol && it<itMax
            qH         .= -D*diff(H)/dx            # flux
            dHdtau       .= -(H[2:end-1] - Hold[2:end-1])/dt - diff(qH)/dx + damp*dHdtau # damped rate of change
    ...
    # Visualize
    plot(xc, 0*H0, linewidth=3); display(plot!(xc, H.-Hana, legend=false, framestyle=:box, linewidth=3, xlabel="lx", ylabel="H", title="damped diffusion (niter=$it)"))
